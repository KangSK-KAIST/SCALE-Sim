...
def gen_read_trace(
    ...
    for r in range(dim_rows):
        ...
        if r < e2:
            # Clock offset takes care of the skew due to store and forward
            clk_offset = r * -1
        else:
    ...
    while(ifmap_done == False) or (filt_done == False):
        ...
        for r in range(dim_rows):
            ...
            if(row_clk_offset[r] >= 0):     # Take care of the skew
                ...
                ifmap_addr = row_base_addr[r] + \
                    addr_row_offset + addr_col_offset
            ...
            # Completed MAC for one OFMAP px
            if (row_clk_offset[r] > 0) and (row_clk_offset[r] % r2c == 0):
...
def gen_write_trace(
        ...
        num_filters=4,
        ofmap_base=2000000,
        # The number of pixels in a convolution window
        conv_window_size=9,
        sram_write_trace_file="sram_write.csv"
):
