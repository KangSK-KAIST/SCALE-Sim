...
def sram_traffic(
    ...
    for px in range(int(e2)):
        addr = int(px / E_w) * strides * hc + (px %
                                               E_w) * strides * num_channels
    ...
    for v in tqdm(range(int(num_v_fold))):
        ...
        cols_this_fold = int(
            min(remaining_cols, max_parallel_window * dimension_cols))
        idx_start = done
        ...
        if num_h_fold > 1:
            ...
            for h in range(num_h_fold):
                ...
                util_this_fold = (rows_this_fold * cols_this_fold) / \
                    (dimension_rows * dimension_cols)
...
def gen_trace_ofmap(
    ...
    # This list has the indices of ofmap px on one ofmap
    ofmap_px_index_list = []
